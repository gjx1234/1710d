...
public class RandomUtil {
	...
	//获得随机0-9之间指定个数的的数字的数组
	public static int[] getRandomNumber(int sub) {
		int[] arr=new int[sub];
		for(int i=0;i<sub;i++) {
	...
	public static int getRandomNumber1(int sub) {
		Random random = new Random();
		int num = random.nextInt(sub);
		return num;
	}
	//获得一个区间内的n个不重复的随机数字   数组 如3-15
	public static int[] getRandomNumber(int min,int max,int sub) {
		int[] arr=new int[sub];
	...
	//获得n位随机英文和数字字符串
	public static String getRandomNumberString(int subs) {
		Random random = new Random();
		StringBuffer buffer = new StringBuffer();
	...
	//获得多个中文字符
	public static String getRandomChineseString(int sub) {
		StringBuffer buffer=new StringBuffer();
		for(int i=0;i<sub;i++) {
			buffer.append(getRandomChineseString());
		}
		return buffer.toString();
	}
	...
	public static String getRandomChineseName() {
		...
		String name1=surname[getRandomNumber()];
		String name2=getRandomChineseString(getRandomNumber(1,2));
		return name1+name2;
	}
